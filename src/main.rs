...
fn main() {
     ...
     *
     * Exceptions i.e. "Expectations"
     *  To handle potential failures the `expect` call makes use of the value
     *  returned by the Result instance.
     *
     *  If the instance of Result is an `Err` value, `expect` will cause the
     *  program to crash and display the message that was passed as an argument
     *  to `expect`
     *
     *  If the instance of Result is an `Ok` value, `expect` will take the
     ...
     *  user's input.
     *
     *  If you do not call `expect`, the program will compile but a warning will
